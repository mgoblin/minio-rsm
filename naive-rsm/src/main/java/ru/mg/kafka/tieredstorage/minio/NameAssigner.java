...
public class NameAssigner {
    ...
    private String assignObjectName(final RemoteLogSegmentMetadata remoteLogSegmentMetadata) {
        final long endOffset = remoteLogSegmentMetadata.endOffset();

        try {
            Validate.notBlank(topicName, "topic name should be not blank");
            Validate.inclusiveBetween(0, Integer.MAX_VALUE, partition,
                    "offsetIndexName should be greater or equals to zero");
            Validate.inclusiveBetween(0, Long.MAX_VALUE, endOffset,
                    "endOffset should be greater or equals to zero");
        } catch (final NullPointerException e) {
            throw new IllegalArgumentException(e.getMessage(), e);
        }

        return String.format(OBJECT_PATH_FORMAT, topicName, partition, endOffset);
    ...
    public String indexNameByType(final RemoteStorageManager.IndexType indexType) {
        try {
            ...
            return switch (indexType) {
                ...
                case PRODUCER_SNAPSHOT -> producerSnapshotIndexObjectName();
                case TRANSACTION -> transactionIndexObjectName();
                case LEADER_EPOCH -> leaderEpochIndexObjectName();
            };
        } catch (final NullPointerException e) {
